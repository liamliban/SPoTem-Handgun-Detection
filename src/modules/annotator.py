...
# return person label list = [person 0, person 1,...,person N] Ex. [False, True]
# gun box is [x_min,y_min,x_max,y_max]
def annotate_frame(hand_regions_in_frame, gun_box, num_of_person):
    ...
    print("persons label: " , persons_label)
    return persons_label

# return the gun box [x_min,y_min,x_max,y_max] from the annotation of MGD
# return None if annotation file does not exist
def get_gunbox_MGD(image_filename, annotation_folder):
    base_name, _ = os.path.splitext(image_filename)
    annotation_file = annotation_folder + base_name + ".xml"
...
def create_MGD_vid_annotation(dataset_folder, data_folder, video_name, output_folder, annotation_folder):
    ...
    for frame_num in range(num_of_frames):
        ...
        gun_box = get_gunbox_MGD(frame_filename, annotation_folder)

        persons_label = annotate_frame(hand_regions_in_frame, gun_box, num_of_person)
